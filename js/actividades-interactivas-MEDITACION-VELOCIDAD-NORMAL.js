...
// Exportar función de celebración
window.celebrarLogro = function() {
    ...
    const mensajes = [
        ...
        '✨ ¡Has capturado la esencia de tu lugar especial!',
        '🌟 ¡Tu creatividad no tiene límites!',
        '🎨 ¡Eres un verdadero artista de las emociones!',
        '💫 ¡Has creado un tesoro emocional único!',
        '🏠 ¡Tu lugar especial ahora vive en tu arte!'
    ];
    
    const mensaje = mensajes[Math.floor(Math.random() * mensajes.length)];
    console.log('📢 Mostrando mensaje:', mensaje);
    
    // Mostrar mensaje con alert estilizado
    alert(mensaje);
    ...
    setTimeout(() => {
        for (let i = 0; i < 30; i++) {
            setTimeout(() => {
                ...
                confeti.textContent = ['🎉', '✨', '🌟', '💫', '🎊', '🏠', '💖'][Math.floor(Math.random() * 7)];
...
// Función auxiliar para crear efecto de celebración
function crearEfectoCelebracion() {
    // Agregar estilo de animación si no existe
    if (!document.getElementById('confeti-animation-style')) {
        const style = document.createElement('style');
        style.id = 'confeti-animation-style';
        style.textContent = `
...
console.log('✅ Función de celebración exportada');

// Exportar funciones auxiliares de celebración
window.revisarViaje = function() {
    console.log('📚 Revisando el viaje completo...');
    alert('📚 ¡Pronto podrás revisar todo tu viaje creativo!');
};

window.nuevaAventura = function() {
    console.log('🔄 Iniciando nueva aventura...');
    if (confirm('🔄 ¿Quieres comenzar una nueva aventura? (Esto te llevará al menú principal)')) {
        window.location.href = 'index.html';
    }
};

window.compartirExperiencia = function() {
    console.log('📤 Compartiendo experiencia...');
    alert('📤 ¡Pronto podrás compartir tu experiencia creativa!');
};

console.log('✅ Funciones auxiliares de celebración exportadas');
...
function mostrarFaseActividad(actividadId, faseIndex) {
    ...
    switch (fase.tipo) {
        ...
        case 'narrativo':
            ...
            break;
        case 'sintesis':
            contenidoHTML = generarFaseSintesis(fase);
            break;
        case 'galeria':
...
function generarFaseNarrativa(fase) {
    // Si es la fase de "Galería de Historias" (actividad 3)
    if (fase.id === 'galeria-historias') {
        return `
            <div class="fase-narrativa max-w-4xl mx-auto">
                ...
                <!-- El Río de Palabras Mágicas -->
                <div class="rio-palabras bg-gradient-to-br from-amber-50 to-orange-50 p-6 rounded-2xl border-2 border-amber-200 mb-6">
                    <h3 class="text-xl font-bold text-amber-700 mb-4 flex items-center gap-2">
                        <span class="text-2xl">🌊</span> ${fase.contenido.mision || 'El Río de Palabras Mágicas'}
                    </h3>
                    
                    <p class="text-gray-700 mb-4">${fase.contenido.mision}</p>
                    
                    <!-- Preguntas de inspiración -->
                    ${fase.contenido.preguntasInspiracion && fase.contenido.preguntasInspiracion.length > 0 ? `
                    <div class="preguntas-activadoras bg-white p-4 rounded-xl border-2 border-amber-300 mb-6">
                        <h4 class="font-bold text-amber-700 mb-3">💡 Para inspirarte:</h4>
                        <ul class="space-y-2">
                            ${fase.contenido.preguntasInspiracion.map(pregunta => `
                    ...
                    <div class="escritura-container bg-white p-6 rounded-xl border-2 border-amber-300">
                        <textarea id="rio-palabras" placeholder="Observa la obra y deja que te cuente su historia..." class="w-full p-4 border-2 border-amber-200 rounded-lg resize-none h-40 text-gray-700 text-lg leading-relaxed"></textarea>
                        
                        <div class="estadisticas-escritura flex justify-between items-center mt-4 text-sm text-gray-600">
                            <span>Palabras escritas: <span id="contador-palabras" class="font-bold text-amber-600">0</span></span>
                            <span>Líneas: <span id="contador-lineas" class="font-bold text-amber-600">0</span></span>
                        </div>
                    </div>
                </div>
                
                ${fase.contenido.celebracionFinal ? `
                <div class="celebracion-final bg-gradient-to-r from-purple-50 to-pink-50 p-6 rounded-2xl border-2 border-purple-200">
                    <h3 class="text-xl font-bold text-purple-700 mb-4">${fase.contenido.celebracionFinal.titulo}</h3>
                    <p class="text-gray-700">${fase.contenido.celebracionFinal.descripcion}</p>
                </div>
                ` : ''}
            </div>
        `;
    ...
    // Si es el "Río de Palabras" original (actividad 2)
    return `
        <div class="fase-narrativa max-w-4xl mx-auto">
            <div class="narrativa mb-6">
                <h2 class="text-2xl font-bold text-primary mb-4">${fase.titulo}</h2>
            ...
            <div class="rio-palabras bg-gradient-to-br from-cyan-50 to-blue-50 p-6 rounded-2xl border-2 border-cyan-200 mb-6">
                ...
                ${fase.contenido.preguntasActivadoras && fase.contenido.preguntasActivadoras.length > 0 ? `
...
/**
 * Generar contenido para fase de síntesis (La Llave del Tesoro)
 */
function generarFaseSintesis(fase) {
    return `
        <div class="fase-sintesis max-w-4xl mx-auto">
            <div class="narrativa mb-6">
                <h2 class="text-2xl font-bold text-primary mb-4">${fase.titulo}</h2>
                <p class="text-lg text-gray-700 mb-6">${fase.contenido.narrativa}</p>
            </div>
            
            <!-- Área de síntesis - La Llave del Tesoro -->
            <div class="llave-tesoro bg-gradient-to-br from-yellow-50 to-amber-50 p-8 rounded-2xl border-2 border-yellow-300 mb-6">
                <h3 class="text-xl font-bold text-amber-700 mb-4 flex items-center gap-2">
                    <span class="text-3xl">🗝️</span> La Llave del Tesoro
                </h3>
                
                <p class="text-gray-700 mb-6">${fase.contenido.mision}</p>
                
                <!-- Resumen de palabras del río -->
                <div class="resumen-rio bg-white p-6 rounded-xl border-2 border-cyan-300 mb-6">
                    <h4 class="font-bold text-cyan-700 mb-3 flex items-center gap-2">
                        <span class="text-xl">🌊</span> Tu Río de Palabras
                    </h4>
                    <div id="palabras-rio-resumen" class="text-gray-600 italic p-4 bg-cyan-50 rounded-lg">
                        Las palabras que escribiste antes aparecerán aquí como recordatorio...
                    </div>
                </div>
                
                <!-- Área para crear la frase esencia -->
                <div class="creacion-frase bg-white p-6 rounded-xl border-2 border-yellow-400 shadow-lg">
                    <h4 class="font-bold text-yellow-700 mb-4 flex items-center gap-2">
                        <span class="text-xl">✨</span> Crea tu Frase Esencia
                    </h4>
                    
                    <p class="text-sm text-gray-600 mb-4">
                        De todas las palabras de tu río, extrae lo más importante y crea una frase que capture 
                        la esencia de tu lugar especial. Puede ser poética, simple, profunda... ¡tú decides!
                    </p>
                    
                    <div class="input-frase-container">
                        <textarea id="frase-esencia" 
                                  placeholder="Mi lugar especial es..." 
                                  class="w-full p-4 border-2 border-yellow-300 rounded-lg resize-none h-24 text-gray-700 text-lg leading-relaxed focus:border-yellow-500 focus:ring-2 focus:ring-yellow-200 transition-all"
                                  maxlength="200"></textarea>
                        
                        <div class="flex justify-between items-center mt-2 text-sm text-gray-600">
                            <span>Caracteres: <span id="contador-caracteres" class="font-bold text-yellow-600">0</span>/200</span>
                            <button onclick="guardarFraseEsencia()" 
                ...
                <div class="ejemplos-frase bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-xl border border-purple-200 mt-6">
                    <h4 class="font-bold text-purple-700 mb-3 text-sm">💡 Ejemplos inspiradores:</h4>
                    <div class="grid md:grid-cols-2 gap-3 text-sm">
                        <div class="ejemplo-item bg-white p-3 rounded-lg border border-purple-200">
                            <p class="text-gray-700 italic">"Mi lugar es un abrazo de colores cálidos donde siempre es domingo"</p>
                        </div>
                        <div class="ejemplo-item bg-white p-3 rounded-lg border border-purple-200">
                            <p class="text-gray-700 italic">"Donde el tiempo se detiene y los sueños tienen olor a pan recién horneado"</p>
                        </div>
                        <div class="ejemplo-item bg-white p-3 rounded-lg border border-purple-200">
                            <p class="text-gray-700 italic">"Mi refugio secreto está hecho de risas, luz de tarde y memorias doradas"</p>
                        </div>
                        <div class="ejemplo-item bg-white p-3 rounded-lg border border-purple-200">
                            <p class="text-gray-700 italic">"El lugar donde mi corazón baila sin música y mi alma respira tranquila"</p>
                        </div>
                    </div>
                </div>
                
                <!-- Vista previa de la frase con estilo artístico -->
                <div id="vista-previa-frase" class="vista-previa-frase hidden bg-gradient-to-br from-yellow-100 to-amber-100 p-8 rounded-2xl border-2 border-yellow-400 mt-6 text-center">
                    <span class="text-4xl mb-4 block">🏆</span>
                    <h4 class="font-bold text-amber-800 mb-4">Tu Frase Esencia:</h4>
                    <p id="frase-guardada" class="text-2xl font-bold text-gray-800 italic leading-relaxed"></p>
                    <p class="text-sm text-amber-700 mt-4">¡Has capturado la esencia de tu lugar especial! ✨</p>
                </div>
            </div>
        </div>
    `;
...
function configurarFaseNarrativa() {
    ...
    if (riopalabras) {
        ...
        const textoGuardado = localStorage.getItem('rio-palabras-texto');
        if (textoGuardado) {
            riopalabras.value = textoGuardado;
        }
        ...
        riopalabras.addEventListener('input', function() {
            ...
            localStorage.setItem('rio-palabras-texto', texto);
        ...
        if (textoGuardado) {
            const palabras = textoGuardado.trim().split(/\s+/).filter(palabra => palabra.length > 0);
...
/**
 * Configurar fase de síntesis
 */
function configurarFaseSintesis() {
    // Recuperar texto del río de palabras del localStorage
    const textoRio = localStorage.getItem('rio-palabras-texto');
    if (textoRio) {
        const resumenDiv = document.getElementById('palabras-rio-resumen');
        if (resumenDiv) {
            // Mostrar las primeras 200 caracteres del río
            const resumenTexto = textoRio.length > 200 ? textoRio.substring(0, 200) + '...' : textoRio;
            resumenDiv.textContent = resumenTexto;
        }
    }
    
    // Configurar contador de caracteres para la frase
    const fraseEsencia = document.getElementById('frase-esencia');
    if (fraseEsencia) {
        fraseEsencia.addEventListener('input', function() {
            const caracteresActuales = this.value.length;
            document.getElementById('contador-caracteres').textContent = caracteresActuales;
            ...
            if (caracteresActuales > 180) {
                contadorSpan.classList.add('text-red-600');
                contadorSpan.classList.remove('text-yellow-600');
            } else {
                contadorSpan.classList.add('text-yellow-600');
                contadorSpan.classList.remove('text-red-600');
            }
        });
        
        // Cargar frase guardada si existe
        const fraseGuardada = localStorage.getItem('frase-esencia-lugar');
        if (fraseGuardada) {
            fraseEsencia.value = fraseGuardada;
            mostrarVistaPrevia(fraseGuardada);
        }
    }
}

/**
 * Guardar frase esencia
 */
window.guardarFraseEsencia = function() {
    const fraseEsencia = document.getElementById('frase-esencia');
    if (!fraseEsencia || !fraseEsencia.value.trim()) {
        alert('✍️ Por favor escribe tu frase esencia antes de guardar');
        return;
    }
    
    const frase = fraseEsencia.value.trim();
    
    // Guardar en localStorage
    localStorage.setItem('frase-esencia-lugar', frase);
    
    // Mostrar vista previa
    mostrarVistaPrevia(frase);
    
    // Notificación de éxito
    alert('✨ ¡Frase guardada con éxito! Tu esencia ha sido capturada.');
    
    console.log('💾 Frase esencia guardada:', frase);
};

/**
 * Mostrar vista previa de la frase
 */
function mostrarVistaPrevia(frase) {
    const vistaPrevia = document.getElementById('vista-previa-frase');
    const fraseGuardada = document.getElementById('frase-guardada');
    
    if (vistaPrevia && fraseGuardada) {
        fraseGuardada.textContent = `"${frase}"`;
        vistaPrevia.classList.remove('hidden');
        
        // Animación de aparición
        vistaPrevia.style.opacity = '0';
        vistaPrevia.style.transform = 'scale(0.9)';
        setTimeout(() => {
            vistaPrevia.style.transition = 'all 0.5s ease-out';
...
function mostrarSiguienteTextoMeditacion() {
    ...
    if (indice < textos.length) {
        ...
        // Tiempos ajustados para velocidad normal de voz
        const tiempos = [
            8000,   // Presentación de Luna (8 segundos)
            9000,   // Respiración guiada (9 segundos)
            8000,   // Puerta dorada (8 segundos)
            11000,  // Llegada al lugar (11 segundos)
            13000,  // Observar colores (13 segundos)
            14000,  // Escuchar sonidos (14 segundos)
            16000,  // Sentir texturas y aromas (16 segundos)
            12000   // Mensaje final (12 segundos)
        ];
        
        const tiempoEspera = tiempos[indice] || 10000;
...
function hablarTextoCompleto(texto) {
    if ('speechSynthesis' in window && !MeditacionState.cancelarVoz) {
        ...
        // Configuración con velocidad normal para que se entienda bien
        utterance.rate = 0.85;       // Velocidad normal-pausada (más natural)
        utterance.pitch = 0.9;       // Tono más grave y cálido (femenino natural)
        utterance.volume = 0.9;      // Volumen claro y audible
