...
function cargarSimuladorManchaAvanzado() {
    console.log('🎨 Iniciando carga del simulador avanzado...');
    
    const container = document.getElementById('herramientas-mancha-container');
    
    if (!container) {
        console.error('❌ Container del simulador no encontrado: herramientas-mancha-container');
        return;
    }
    ...
    const inicializar = () => {
        console.log('🔧 Intentando inicializar simulador...');
        console.log('   SimuladorManchaAvanzado disponible:', !!window.SimuladorManchaAvanzado);
        console.log('   inicializarSimuladorMancha disponible:', !!window.inicializarSimuladorMancha);
        
        // Método 1: Usar el sistema global del simulador
        if (window.SimuladorManchaAvanzado && typeof window.SimuladorManchaAvanzado.inicializar === 'function') {
            console.log('✅ Usando SimuladorManchaAvanzado.inicializar()');
            try {
                window.SimuladorManchaAvanzado.inicializar(container);
                console.log('✅ Simulador inicializado exitosamente');
                return true;
            } catch (error) {
                console.error('❌ Error al inicializar con SimuladorManchaAvanzado:', error);
            }
        }
        
        // Método 2: Usar la función directa de inicialización
        if (typeof window.inicializarSimuladorMancha === 'function') {
            console.log('✅ Usando inicializarSimuladorMancha()');
            try {
                window.inicializarSimuladorMancha('canvas-mancha-avanzado');
                console.log('✅ Simulador inicializado con función directa');
                return true;
            } catch (error) {
                console.error('❌ Error al inicializar con función directa:', error);
            }
        }
        
        console.warn('⚠️ No se pudo inicializar el simulador avanzado, usando fallback');
        return false;
    };

    // Verificar si el simulador ya está disponible
    if (window.SimuladorManchaAvanzado || window.inicializarSimuladorMancha) {
        console.log('✨ Simulador ya disponible, inicializando...');
        if (!inicializar()) {
        ...
        // El simulador debería estar cargado desde index.html
        // Esperar a que se cargue
        console.log('⏳ Esperando a que el simulador se cargue...');
        ...
        const verificarCarga = setInterval(() => {
            intentos++;
            console.log(`   Intento ${intentos}/${maxIntentos} de cargar simulador...`);
            ...
            if (window.SimuladorManchaAvanzado || window.inicializarSimuladorMancha) {
                ...
                console.error('❌ Timeout esperando el simulador, usando fallback');
                clearInterval(verificarCarga);
                mostrarFallbackCanvas(container);
            }
        }, 300);
