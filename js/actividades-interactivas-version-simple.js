...
function mostrarActividad(numeroActividad) {
    ...
    // Verificar que existe el contenedor antes de continuar (laboratorio, guía didáctica u otros)
    const contenedor = document.getElementById('lab-content-container') || 
                       document.getElementById('contenido-actividad') || 
                       document.getElementById('fase-contenido');
    if (!contenedor) {
...
function mostrarFaseActividad(actividadId, faseIndex) {
    ...
    // Buscar contenedor - priorizar lab-content-container (laboratorio), contenido-actividad (guía didáctica) o fase-contenido (otros)
    let contenedor = document.getElementById('lab-content-container') || 
                     document.getElementById('contenido-actividad') || 
                     document.getElementById('fase-contenido');
    
    if (contenedor) {
        // Si es el contenedor de la guía didáctica, mostrarlo
        if (contenedor.id === 'contenido-actividad') {
        ...
        if (fase.tipo === 'intro') {
            ...
            contenidoHTML = `
                <div class="fase-actividad">
                    ...
                    <!-- SIMULADOR DE MANCHAS INTERACTIVO -->
                    ${fase.id.includes('mancha') || actividadId === 'actividad4' ? `
                        <div class="simulador-manchas" style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin: 20px 0;">
                            <h3>🎨 Simulador de Manchas Interactivo</h3>
                            <canvas id="canvas-manchas" width="400" height="300" style="border: 2px solid #ddd; background: white; cursor: crosshair; border-radius: 8px;"></canvas>
                            <div style="margin-top: 15px;">
                                <button onclick="activarHerramientaMancha('pincel')" style="background: #007bff; color: white; border: none; padding: 10px 15px; margin: 5px; border-radius: 5px; cursor: pointer;">🖌️ Pincel</button>
                                <button onclick="activarHerramientaMancha('agua')" style="background: #17a2b8; color: white; border: none; padding: 10px 15px; margin: 5px; border-radius: 5px; cursor: pointer;">💧 Agua</button>
                                <button onclick="activarHerramientaMancha('gotero')" style="background: #28a745; color: white; border: none; padding: 10px 15px; margin: 5px; border-radius: 5px; cursor: pointer;">💉 Gotero</button>
                                <button onclick="limpiarManchaPersonal()" style="background: #6c757d; color: white; border: none; padding: 10px 15px; margin: 5px; border-radius: 5px; cursor: pointer;">🧹 Limpiar</button>
                                <button onclick="guardarManchaPersonal()" style="background: #28a745; color: white; border: none; padding: 10px 15px; margin: 5px; border-radius: 5px; cursor: pointer;">💾 Guardar</button>
                            </div>
                        </div>
                    ` : ''}
                    
                    <!-- MEDITACIÓN INTERACTIVA -->
                    ${fase.id.includes('meditacion') || actividadId === 'actividad2' ? `
                        <div class="meditacion-interactiva" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 25px; border-radius: 15px; margin: 20px 0; color: white;">
                            <h3>🧘‍♀️ Meditación Guiada</h3>
                            <p>Prepárate para un momento de conexión interior antes de crear.</p>
                            <div style="margin-top: 15px;">
                                <button onclick="iniciarMeditacion()" style="background: rgba(255,255,255,0.2); color: white; border: 2px solid white; padding: 15px 25px; margin: 5px; border-radius: 25px; cursor: pointer; font-weight: bold;">▶️ Iniciar Meditación</button>
                                <button onclick="pausarMeditacion()" style="background: rgba(255,255,255,0.2); color: white; border: 2px solid white; padding: 15px 25px; margin: 5px; border-radius: 25px; cursor: pointer;">⏸️ Pausar</button>
                                <button onclick="saltarMeditacion()" style="background: rgba(255,255,255,0.1); color: white; border: 1px solid rgba(255,255,255,0.5); padding: 10px 20px; margin: 5px; border-radius: 20px; cursor: pointer;">⏭️ Saltar</button>
                            </div>
                            <div id="meditacion-estado" style="margin-top: 15px; font-style: italic;">Presiona "Iniciar Meditación" cuando estés listo...</div>
                        </div>
                    ` : ''}
...
// ===============================
// FUNCIONES INTERACTIVAS ESPECÍFICAS
// ===============================

// Simulador de Manchas
function activarHerramientaMancha(herramienta) {
    console.log('🎨 Activando herramienta:', herramienta);
    
    const canvas = document.getElementById('canvas-manchas');
    if (!canvas) {
    ...
    mostrarNotificacion(`Herramienta ${herramienta} activada`);
}
...
function limpiarManchaPersonal() {
    ...
    if (canvas) {
        ...
        mostrarNotificacion('Canvas limpiado');
    }
}
...
function guardarManchaPersonal() {
    ...
    if (canvas) {
        ...
        link.download = 'mi-mancha-creativa.png';
        link.href = canvas.toDataURL();
        link.click();
        mostrarNotificacion('Mancha guardada correctamente');
    }
}

// Sistema de Meditación
function iniciarMeditacion() {
    console.log('🧘‍♀️ Iniciando meditación...');
    const estado = document.getElementById('meditacion-estado');
    
    const textosMeditacion = [
        'Cierra los ojos suavemente...',
        'Respira profundamente... inhala... exhala...',
        'Siente tu cuerpo relajándose...',
        'Conecta con tu creatividad interior...',
        'Imagina un lugar sereno donde puedas crear...',
    ...
    function siguienteTexto() {
        if (!MeditacionState.activa || indice >= textosMeditacion.length) {
            if (estado) estado.textContent = 'Meditación completada. ¡Ahora crea!';
            return;
        }
        
        if (estado) estado.textContent = textosMeditacion[indice];
        indice++;
        
        setTimeout(siguienteTexto, 3000);
    }
    
    siguienteTexto();
    mostrarNotificacion('Meditación iniciada. Relájate y sigue las instrucciones.');
}

function pausarMeditacion() {
    MeditacionState.activa = false;
    const estado = document.getElementById('meditacion-estado');
    if (estado) estado.textContent = 'Meditación pausada. Presiona "Iniciar" para continuar.';
    mostrarNotificacion('Meditación pausada');
}

function saltarMeditacion() {
    MeditacionState.activa = false;
    const estado = document.getElementById('meditacion-estado');
    if (estado) estado.textContent = 'Meditación omitida. ¡Comienza a crear directamente!';
    mostrarNotificacion('Meditación omitida. ¡Adelante con la creatividad!');
}
